...
import { handleCloseAllModal, handleOpenLoginModal, handleOpenSignUpFormModal, modalSelector } from "../../../redux/modal/modalSlice";
...
const SignUpOtpModal = () => {
	...
	return (
		<>
			<Dialog open={signUpOtpShow} onShow={() => setOtp(["", "", "", "", "", ""])} onClose={() => dispatch(handleCloseAllModal())}>
							...
							<div className="login-right">
								<form>
									...
									<div className="otp-field">
										{otp.map((digit, index) => (
											<input key={index} type="text" value={digit} maxLength="1" onChange={(event) => handleInputChange(index, event)} ref={inputRefs[index]} />
										))}
									</div>
